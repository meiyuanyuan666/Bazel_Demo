...
Status MPCController::Init(std::shared_ptr<DependencyInjector> injector,
  ...
  if (basic_state_size_ != q_param_size) {
    ...
    //                  basic_state_size_);
    std::string error_msg = "MPC controller error";
    AERROR << error_msg;
...
Status MPCController::ComputeControlCommand(
  ...
  // 获取操作系统当前时间点（精确到微秒）     
  chrono::time_point<chrono::system_clock, chrono::microseconds> mpc_start_tpMicro
          = chrono::time_point_cast<chrono::microseconds>(chrono::system_clock::now());

  // (各种精度的)时间点 => (秒精度的)时间戳
  double mpc_start_timestamp = std::chrono::system_clock::to_time_t(mpc_start_tpMicro);
  ...
  // 获取操作系统当前时间点（精确到微秒）     
  chrono::time_point<chrono::system_clock, chrono::microseconds> mpc_end_tpMicro
          = chrono::time_point_cast<chrono::microseconds>(chrono::system_clock::now());

  // (各种精度的)时间点 => (秒精度的)时间戳
  double mpc_end_timestamp = std::chrono::system_clock::to_time_t(mpc_end_tpMicro);
...
void MPCController::ComputeLongitudinalErrors(
  ...
  // const double current_control_time = Clock::NowInSeconds();
      // 获取操作系统当前时间点（精确到微秒）     
  chrono::time_point<chrono::system_clock, chrono::microseconds> current_control_tpMicro
          = chrono::time_point_cast<chrono::microseconds>(chrono::system_clock::now());

  // (各种精度的)时间点 => (秒精度的)时间戳
  const double current_control_time = std::chrono::system_clock::to_time_t(current_control_tpMicro);
